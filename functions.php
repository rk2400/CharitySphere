...
function get_total_price_by_month($con,$date){
  $raised=0;
      ...
      $resp=mysqli_query($con,"SELECT `donations`.`amount` from `donations` where month(`donations`.`added_on`)=month('$date') and year(`added_on`)=year('$date')");
...
function get_total_price_by_date($con,$date){
  $raised=0;
  $resp=mysqli_query($con,"SELECT `donations`.`amount` from `donations` where `donations`.`added_on` = '$date' ");
...
function get_total_price_by_year($con,$date){
  $raised=0;
      $resp=mysqli_query($con,"SELECT `donations`.`amount` from `donations` where year(`added_on`)=year('$date')");
    while($row=mysqli_fetch_assoc($resp)){
       $raised=$raised+$row['amount'];
    }
    return $raised;
...
function get_total_donations($con){
  $raised=0;
      $resp=mysqli_query($con,"SELECT `donations`.`amount` from `donations` where `donations`.`payment_id`!=''");
    while($row=mysqli_fetch_assoc($resp)){
       $raised=$raised+$row['amount'];
    }
    return $raised;
